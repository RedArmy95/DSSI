...
sd1 = sd(x.a)
mean1 = mean(x.a)
res = x.a[ x.a > mean1 - 2 * sd1 & x.a < mean1 + 2 * sd1]
res
...
x.b[c(-1,-2,-length(x.b),-length(x.b))]
```
...
cat("This is",x.b[1])
message(x.b)
#print用於輸出單一「物件」，cat可連續輸出，但無法輸出matrix等包含兩個以上值的物件，message則可連續輸出物件中的值，且會highlight，多用於編寫package或function。
```
...
#先照前面的a再照後面的b，負號代表decreasing order
```
...
df3 <- data.frame(country,import,export)
df3 <- df3[order(-df3$export,-df3$import),]
df3
```
